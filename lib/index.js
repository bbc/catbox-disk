...
exports = module.exports = internals.Connection = function DiskCache (options) {
    ...
    // This below is a hack, for some reason catbox-disk filesystem checking takes a long while
    // And our requests kick in before the checks have chance to finish, this means our cache has a state of disconnected.
    // New versions should wait for the promise to resolve before it starts being used but we can't use an updated version of Catbox just yet
    // So this will have to do for now, this is an assumption our tmp folder is ready to use (which it is)
